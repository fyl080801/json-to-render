...
export function resolveToTypeWrapper(
  ...
  if (
    (typeOrIdentifier && typeof typeOrIdentifier === 'string') ||
  ...
  if (typeOrIdentifier && typeof typeOrIdentifier === 'function') {
    ...
    typeWrapper = {
      eagerType: null,
      lazyType: () => (typeOrIdentifier as CallableFunction)(),
    }
  }
  ...
  if (!typeOrIdentifier && propertyName) {
    const identifier = (Reflect as any).getMetadata(
      'design:type',
      target,
      propertyName
    )
  ...
  if (
    ...
    const paramTypes: ServiceIdentifier[] = (Reflect as any).getMetadata(
      'design:paramtypes',
      target,
      propertyName
    )
